...
class GUIControlWindows():
    def __init__(self, window_name, exe_dir, exe_cmd, exe_sleep=0.1):
        # variables
    ...
    def execute_application(self):
        parent_handle = win32gui.FindWindow(None, self.window_name)  # check whether software has already been running
        if parent_handle > 0:
    ...
    def make_window_active(self, window_name):
        ...
        if parent_handle == 0:
        ...
        elif parent_handle > 0 :
            print('Active window has changed to ', window_name)
            # get window position
            self.active_window_x1,self.active_window_y1,self.active_window_x2,self.active_window_y2 = win32gui.GetWindowRect(parent_handle)
            ...
            # get window info
            self.active_window_name = win32gui.GetWindowText(parent_handle)
    ...
    def close_active_window(self, offset_x=0, offset_y=0):
        ...
        click_pos_x = window.left + window.width - 1 + offset_x
        click_pos_y = window.top + 1 + offset_y
        print('click pos: (%s, %s).' % (click_pos_x, click_pos_y))
        print('')

        pag.click(click_pos_x, click_pos_y)
    ...
    def click_by_pos_on_window(self, window_name, offset_x, offset_y, clicks):
        ...
        if self.make_window_active(window_name):
            ...
            print('%s window is not activated' % window_name)
